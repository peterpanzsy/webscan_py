...
class sqldetect(threading.Thread):
...
if __name__=="__main__":
    ...
    if ch=="s":
        ...
        crawlerth=crawler(num,firstlink)
        crawlerth.crawmain()
        print("begin scanning! ")
        if glovar.overflag==1:
            ch=input("G:Detect hreflist; P:Detect formlist")
            if ch=="G":
                for hrefdict in hreflist:#only single thread otherwise console can not deal the output
                    sqldetect(hrefdict).hrefInject(hrefdict)
            if ch=="P":
                for formdict in formlist:
